...
export default function FormEditor() {
  ...
  return (
    <article
      className={`grid  h-screen w-full ${form && !form.message && "grid-cols-[250px_1fr_300px]"}`}
    >
      {form && !form.message ? (
        <>
          {/* Sidebar */}
            ...
            {selectedField ? (
              <>
                <h3 className="text-xl font-semibold">
                  Editing Field: {selectedField.label}
                </h3>
                <Input
                  placeholder="Field Label"
                  value={selectedField.label}
                  onChange={(e) =>
                    setSelectedField((prev: Field) => ({
                      ...prev,
        ...
        <article className="w-full h-full flex flex-col justify-center items-center gap-2">
          <h2 className="text-xl text-slate-500">
            An error occured, form not found
          </h2>
          <Button
            className="bg-orange-500 hoverLbg-orange-700 text-white"
            asChild
          >
