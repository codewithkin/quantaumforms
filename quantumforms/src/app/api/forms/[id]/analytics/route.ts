...
export async function GET(
  ...
  try {
    ...
    const avgTimeTaken =
      form.responses.reduce((acc, r) => acc + (r.timeTaken || 0), 0) /
        totalResponses || 0;

    // Calculate device distribution
    const deviceStats = form.responses.reduce(
      (acc, r) => {
    ...
    const locationStats = form.responses.reduce(
      (acc, r) => {
    ...
    const responseTrends = form.responses.reduce(
      (acc, r) => {
